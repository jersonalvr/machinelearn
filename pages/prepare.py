...
def show_prepare():
    ...
    if not selected_columns:
        st.info("👆 Selecciona una o más columnas para ver su análisis detallado.")
            ...
            for tab, col in zip(tabs, selected_columns):
                with tab:
                    try:
                        st.markdown(f"### Análisis de {col} ({st.session_state.er_data[col].dtype})")
                        
                        # Safely convert column to handle mixed types
                        column_data = st.session_state.er_data[col].fillna('Sin valor').astype(str)
                        ...
                        with col2:
                            st.write(f"Valores nulos: {st.session_state.er_data[col].isnull().sum()}")
                        with col3:
                            st.write(f"% nulos: {(st.session_state.er_data[col].isnull().sum() / len(st.session_state.er_data) * 100).round(2)}%")
                        ...
                        freq_df = pd.DataFrame({
                            ...
                            'Porcentaje': (value_counts.values / len(st.session_state.er_data) * 100).round(2)
                        })
        ...
        if st.button("Eliminar columnas seleccionadas"):
            try:
                ...
                if new_null_count <= current_null_count:  # Permitir igual o menor cantidad de nulos
                    ...
                    if st.checkbox("Ver lista de columnas restantes"):
    ...
    if not missing_df.empty:
        ...
        if st.button("Aplicar estrategia de valores faltantes"):
            try:
                ...
                if missing_strategy == "Eliminar filas":
                    ...
                    if prepare_cleaned.isnull().sum().sum() == 0:
                        ...
                        st.error(f"Error: Aún quedan {prepare_cleaned.isnull().sum().sum()} valores nulos después de la eliminación.")
                        return
                else:
                ...
                if missing_strategy == "Eliminar filas":
                    rows_removed = nulls_before - nulls_after
                    st.success(f"Se eliminaron {rows_removed} filas con valores faltantes")
                else:
                ...
                if final_null_check == 0:
                    ...
                    st.session_state.temp_prepared_data = prepare.copy()
                    st.session_state.er_data = prepare.copy()
    ...
    with st.expander("Procesamiento de Fechas"):
        ...
        if date_columns:
            date_format = st.selectbox(
                "Formato de fecha",
                ["yyyy-mm-dd", "dd-mm-yyyy", "mm-dd-yyyy", "yyyy-mm-dd hh:mm"]
            )
            
            date_features = st.multiselect(
                "Seleccionar características a extraer",
                ["Año", "Mes", "Día", "Día de la semana", "Trimestre", "Estación", 
                 "Es fin de semana", "Hora del día"]
            )
            
            if st.button("Procesar fechas"):
                for col in date_columns:
                    try:
                        if date_format == "yyyy-mm-dd":
                            prepare[col] = pd.to_datetime(prepare[col], format='%Y-%m-%d')
                        elif date_format == "dd-mm-yyyy":
                            prepare[col] = pd.to_datetime(prepare[col], format='%d-%m-%Y')
                        elif date_format == "mm-dd-yyyy":
                            prepare[col] = pd.to_datetime(prepare[col], format='%m-%d-%Y')
                        else:  # yyyy-mm-dd hh:mm
                            prepare[col] = pd.to_datetime(prepare[col], format='%Y-%m-%d %H:%M')

                        if "Año" in date_features:
                            prepare[f'{col}_año'] = prepare[col].dt.year
                        if "Mes" in date_features:
                            prepare[f'{col}_mes'] = prepare[col].dt.month
                        if "Día" in date_features:
                            prepare[f'{col}_dia'] = prepare[col].dt.day
                        if "Día de la semana" in date_features:
                            prepare[f'{col}_dia_semana'] = prepare[col].dt.dayofweek + 1
                        if "Trimestre" in date_features:
                            prepare[f'{col}_trimestre'] = prepare[col].dt.quarter
                        if "Es fin de semana" in date_features:
                            prepare[f'{col}_fin_semana'] = prepare[col].dt.dayofweek.isin([5, 6]).astype(int)
                        if "Hora del día" in date_features and date_format == "yyyy-mm-dd hh:mm":
                            prepare[f'{col}_hora'] = prepare[col].dt.hour
                            
                        prepare = prepare.drop(columns=[col])
                        st.success(f"Columna {col} procesada")
                    except Exception as e:
    ...
    if len(numeric_columns) > 0:
        ...
        if cols_to_normalize and st.button("Aplicar normalización"):
    ...
    if st.button("Guardar datos preparados"):
        try:
            null_count = prepare.isnull().sum().sum()
            if null_count == 0:
                st.session_state.prepared_data = prepare.copy()
    ...
    st.info("👆 No te olvides de guardar los datos preparados antes de continuar con el análisis en la pagina Training o Test.")